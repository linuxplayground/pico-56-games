; vim: set ft=asm_ca65 ts=4 sw=4 et cc=80 tw=80:
; PICO-56 VDP Library
;
...
; The routines in this library are generic graphics routines for working with
; the TMS9918A.  Not all the features of the TMS9918A are represented here.
;
; Of particular note is the graphics mode used here.  While the routine is
; called vdp_g2_init implying that graphics mode II is used, it's not actually
; graphics mode II.  Well not completely.  For a detailed description of what
; this mode is, see the "Video Display Processors - Programmers Guide" by Texas
; Instruments.  Specifically, section 8.4.2 on page 8-8 "Playing Games with VRAM
; Addressing"
; <<<
; What this mode does is effectively shrink the Graphics II Mode Color and
; Pattern Tables down from Hex 1800 bytes to Hex 800 bytes. This enables us to
; define up to 256 8x8 pixel patterns and 256 corresponding eight byte Color
; Table entries. Color is still mapped onto a pattern exactly as in Graphics II
; Mode. The 768 byte Name Table is not split up into three equal sections as in
; Graphics II Mode but works as in Graphics I Mode. A byte of information
; written anywhere in the Name Table will select the appropriate pattern and the
; corresponding eight byte color entry and place it on the screen. In Appendix C
; can be found the Pattern Graphics Address Location Tables. This mode is useful
; because it provides the memory savings of Graphics I Mode while allowing the
; color detail available in Graphics II Mode. However, a unique pattern for each
; screen position can no longer be defined, which is neccesary for highly
; detailed pictures or for bitmapping the screen. When in this mode 32 sprites
; can no longer be used. If you try to put more than eight sprites on the screen
; at once, they will start to duplicate themselves on the screen.
; >>>
;
; The main reason for choosing this graphics mode is that we can do a
; framebuffer approach to drawing on the screen.  This library maintains a 0x300
; byte buffer in regular memory and the `vdp_flush` routine copies it to VRAM
; during the VBLANK interval.  On stock TMS9918A ICs, this is the time during
; which writes to VRAM are the fastest.  Experiments show that the timings work
; well.
;
; Most of the routines here are working on the framebuffer in regular RAM and
; rely on calls to `vdp_flush` to flush the framebuffer into the VDP.

; *** IMPORTANT NOTE ***
; Note on timings.  These routines have been modified from other versions of
; this library to run as fast as possible which works on the RP2040 in the
; PICO-56.  If this library were to be used on a stock TMS9918A it WILL NOT
; WORK.  The timings are too aggressive.
;
.include "io.inc"
.include "app.inc"
.include "macro.inc"
